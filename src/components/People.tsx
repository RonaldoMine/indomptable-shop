...
export default function People({peoples}: any) {
    ...
    const openModal = (img: string, title: string, index: number, blurUrl: string) => {
        setIsOpen(true);
        setCurrentImage({img: img, title: title, index: index, blurUrl})
    }
    const handlePreviousPage = () => {
        let currentIndexItem = currentImage.index - 1;
        const item = peoples.find((index: number) => index === currentIndexItem);
        if (item) {
            setCurrentImage({
              img: item.src.url,
              title: item.title,
    ...
    return (
      <div className="dark:bg-neutral-800" id="our-community">
        <div className="p-4">
          ...
          <div
            id="masonry-grid"
            className="md:columns-3 lg:columns-4 justify-items-center 2xl:columns-5 sm:columns-2 sm:gap-4"
          >
                ...
                <div className="flex min-h-full items-center justify-center p-4 text-center">
                  ...
                  >
                    <div>
                      ...
                      <div
                        className={
                          "w-full flex justify-around sm:justify-between mt-2"
                        }
                      >
                        <button
                          onClick={handlePreviousPage}
                          className={`bg-white bg-opacity-0 hover:bg-opacity-20 focus:bg-opacity-20  transition ease-in-out duration-500 rounded-full z-[99999] p-4 ${
                        ...
                        <button
                          onClick={closeModal}
                          className={
                            "p-4 rounded-full bg-red-500 bg-opacity-20 border-red-500 border-2 hover:bg-opacity-100 transition-all duration-500"
                          }
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
